...
function Setup2FA() {
  ...
  const handleEnable2FA = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/2fa/enable-2fa?user_id=${userId}`);
  ...
  const handleCopyToClipboard = () => {
    navigator.clipboard.writeText(totpSecret).then(() => {
      setIsCopied(true);
      setTimeout(() => setIsCopied(false), 1000); // Reset animation state after 1s
    });
  };

  const iconStyle = {
    transform: isCopied ? 'scale(1.5)' : 'scale(1)',
